...
import {
  fetchApiAnnouncementOpenTime,
  updateApiAnnouncementOpenTime,
  fetchWebsiteSettings,

  updateWebsiteSettings,
} from '../services/configService.ts';
import { checkApiStatus } from '../services/studentService.ts';
import type { ApiStatus } from '../services/studentService.ts';
import Card from './Card.tsx';
import { ApiStatusIcon, CheckCircleIcon, XCircleIcon, ClockIcon, SchoolLogoIcon } from './icons.tsx';

interface PengaturanPageProps {
  onLogoUpdate: () => void;
}

const PengaturanPage: React.FC<PengaturanPageProps> = ({ onLogoUpdate }) => {
  // API URL State
  ...
  const [saveLogoStatus, setSaveLogoStatus] = useState<{ type: 'success' | 'error'; message: string } | null>(null);

  // Website Settings State
  const [websiteTitleInput, setWebsiteTitleInput] = useState<string>('');
  const [websiteSubtitleInput, setWebsiteSubtitleInput] = useState<string>('');
  const [loadingWebsiteSettings, setLoadingWebsiteSettings] = useState<boolean>(true);
  const [websiteSettingsError, setWebsiteSettingsError] = useState<string | null>(null);
  const [saveWebsiteSettingsStatus, setSaveWebsiteSettingsStatus] = useState<{ type: 'success' | 'error'; message: string; isSubmitting?: boolean } | null>(null);
  ...
  const refreshLogoUrlSettings = () => {
    const currentActiveUrl = getLogoUrl();
    setCurrentLogo(currentActiveUrl);
    const storedUserUrl = localStorage.getItem(USER_LOGO_URL_KEY);
    setLogoUrlInput(storedUserUrl || '');
  };
  ...
  const loadWebsiteSettings = useCallback(async () => {
    setLoadingWebsiteSettings(true);
    setWebsiteSettingsError(null);
    try {
      const settings = await fetchWebsiteSettings();
      setWebsiteTitleInput(settings?.title || '');
      setWebsiteSubtitleInput(settings?.subtitle || '');
    } catch (error) {
      const message = error instanceof Error ? error.message : "Gagal memuat pengaturan website dari API.";
      setWebsiteSettingsError(message);
    } finally {
      setLoadingWebsiteSettings(false);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    initializePage();
  }, [loadAnnouncementTimeFromApi, loadWebsiteSettings]);
  ...
  const handleSaveLogoUrl = () => {
      setSaveLogoStatus(null);
      try {
          if (logoUrlInput.trim() !== '' && !logoUrlInput.trim().startsWith('http')) {
              setSaveLogoStatus({ type: 'error', message: "URL Logo tidak valid. Harus dimulai dengan http:// atau https://." });
              return;
          }
          localStorage.setItem(USER_LOGO_URL_KEY, logoUrlInput.trim());
          setSaveLogoStatus({ type: 'success', message: "URL Logo berhasil disimpan di penyimpanan lokal." });
          onLogoUpdate();
          refreshLogoUrlSettings();
      } catch (error) {
          console.error("Error saving Logo URL to localStorage:", error);
          setSaveLogoStatus({ type: 'error', message: "Gagal menyimpan URL Logo ke penyimpanan lokal." });
      }
  };
  ...
  const handleResetLogoUrl = () => {
      setSaveLogoStatus(null);
      try {
          localStorage.removeItem(USER_LOGO_URL_KEY);
          setLogoUrlInput('');
          setSaveLogoStatus({ type: 'success', message: "Logo telah direset ke konfigurasi default aplikasi." });
          onLogoUpdate();
          refreshLogoUrlSettings();
      } catch (error) {
          console.error("Error resetting Logo URL from localStorage:", error);
          setSaveLogoStatus({ type: 'error', message: "Gagal mereset Logo dari penyimpanan lokal." });
      }
  };
  ...
  const handleSaveWebsiteSettings = async () => {
    setSaveWebsiteSettingsStatus({ message: 'Menyimpan...', type: 'success', isSubmitting: true });
    try {
      await updateWebsiteSettings({
        title: websiteTitleInput,
        subtitle: websiteSubtitleInput,
      });
      setSaveWebsiteSettingsStatus({ 
        type: 'success', 
        message: 'Pengaturan website berhasil disimpan ke server.',
        isSubmitting: false
      });
    } catch (error) {
      const message = error instanceof Error ? error.message : "Gagal menyimpan pengaturan website ke server.";
      setSaveWebsiteSettingsStatus({ type: 'error', message, isSubmitting: false });
    }
  };
      ...
      <Card title="Informasi Konfigurasi">
        <ul className="list-disc list-inside space-y-3 text-sm text-text-secondary">
            <li>Aplikasi ini menggunakan konfigurasi dengan urutan prioritas:
                <ol className="list-decimal list-inside ml-4 mt-1 space-y-2">
                    <li><b>Logo Aplikasi:</b>
                        <ul className="list-disc list-inside ml-4">
                            <li><b>Kustom Pengguna (Penyimpanan Lokal):</b> Jika Anda menyimpan URL Logo melalui form di atas, logo tersebut akan digunakan khusus di peramban ini.</li>
                            <li><b>Konfigurasi Server (<code>public/config.json</code>):</b> Jika tidak ada setelan kustom, aplikasi akan memuat URL logo dari file <code>public/config.json</code>.</li>
                            <li><b>Default Aplikasi (Icon):</b> Jika kedua metode di atas gagal/tidak tersedia, aplikasi akan menggunakan ikon logo default.</li>
                        </ul>
                    </li>
                    <li className="mt-2"><b>Link API:</b>
                        <ul className="list-disc list-inside ml-4">
                            <li><b>Kustom Pengguna (Penyimpanan Lokal):</b> Jika Anda menyimpan Link API, setelan tersebut akan digunakan khusus di peramban ini.</li>
                            <li><b>Konfigurasi Server (<code>public/config.json</code>):</b> Jika tidak ada setelan kustom, aplikasi akan memuat Link API dari <code>public/config.json</code>.</li>
                            <li><b>Default Aplikasi (Hardcode):</b> Jika keduanya gagal, aplikasi akan menggunakan Link API default: <code>{DEFAULT_API_BASE_URL}</code>.</li>
                        </ul>
                    </li>
                    <li className="mt-2"><b>Pengaturan Website Publik:</b>
                        <ul className="list-disc list-inside ml-4">
                            <li>Sepenuhnya dikelola melalui <b>API Server</b>.</li>
                            <li>Judul dan Teks Keterangan yang Anda atur akan disimpan di server dan dapat digunakan oleh halaman pengumuman publik.</li>
                        </ul>
                    </li>
                    <li className="mt-2"><b>Waktu Pengumuman:</b>
                        <ul className="list-disc list-inside ml-4">
                            <li>Sepenuhnya dikelola melalui <b>API Server</b>.</li>
                            <li>Perubahan yang Anda buat di atas akan langsung disimpan ke server.</li>
                        </ul>
                    </li>
                </ol>
            </li>
            <li>Tombol "Reset" akan menghapus setelan kustom dari peramban Anda, mengembalikan penggunaan ke konfigurasi di bawahnya.</li>
            <li>Untuk mengosongkan waktu pengumuman, hapus nilai dari input "Ubah Waktu Pengumuman" dan klik "Simpan Waktu ke Server".</li>
            <li>Contoh isi <code>public/config.json</code>:
