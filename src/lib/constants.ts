    ...
    if (response.ok) {
      ...
      if (config) {
        ...
        if (typeof config.logoUrl === 'string') {
            loadedAppConfig.logoUrl = config.logoUrl.trim();
            console.info('Logo URL loaded from config.json:', loadedAppConfig.logoUrl);
        }
      } else {
         console.warn('config.json is empty or invalid. Using hardcoded defaults.');
      }
    } else {
...
export const getLogoUrl = (): string => {
    try {
        const userDefinedUrl = localStorage.getItem(USER_LOGO_URL_KEY);
        if (userDefinedUrl && userDefinedUrl.trim() !== '') {
            return userDefinedUrl.trim();
        }
    } catch (e) {
        console.error("Error accessing localStorage for Logo URL:", e);
    }

    if (loadedAppConfig.logoUrl && loadedAppConfig.logoUrl.trim() !== '') {
        return loadedAppConfig.logoUrl;
    }

    return DEFAULT_LOGO_URL; // Returns empty string if nothing is set
};
