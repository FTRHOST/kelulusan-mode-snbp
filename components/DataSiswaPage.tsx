...
import { Student, StatusKelulusan, Jurusan } from '../types.ts';
...
const DataSiswaPage: React.FC<DataSiswaPageProps> = ({ students, setStudents }) => {
  ...
  const handleDownloadTemplate = () => {
    ...
    const notes = [
        "# Template Impor Data Siswa",
        "# --------------------------",
        "# Petunjuk Pengisian:",
        "# 1. Gunakan format CSV (Comma Separated Values).",
        "# 2. Pastikan urutan kolom sesuai dengan header. Header case sensitive.",
        "# 3. TanggalLahir: Gunakan format YYYY-MM-DD (contoh: 2005-08-15) atau kosongkan jika tidak ada.",
        `# 4. Jurusan: Isi dengan salah satu dari (${Object.values(Jurusan).join('; ')}) atau kosongkan. Case sensitive.`,
        `# 5. StatusKelulusan: Isi dengan 0 (Tidak Lulus) atau 1 (Lulus).`,
        "# 6. Sekolah, Daerah, Provinsi dapat dikosongkan jika tidak ada datanya.",
        "# 7. Jika ada koma (,) dalam data field (misal nama jurusan), apit dengan tanda kutip ganda (\").",
        "# 8. Baris contoh di bawah header hanya untuk ilustrasi, hapus sebelum mengisi data Anda.",
        "# 9. Baris contoh di bawah header hanya untuk ilustrasi, hapus sebelum mengisi data Anda.",
        "# --------------------------\n"
    ].join("\n");

    const csvContent = `${notes}${headers}\n${exampleRow}\n`;
